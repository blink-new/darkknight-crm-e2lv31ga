...
export function useGoogleCalendar() {
  ...
  const getFunctionsUrl = (): string => {
    ...
    const hardcodedUrl = 'https://zgqtllbobikwychknsuu.supabase.co/functions/v1';
  ...
  const checkConnection = useCallback(async (silent = false) => {
    try {
      ...
      console.log(`Checking Google Calendar connection status at: ${apiUrl}/google-calendar/status`);
      
      try {
        const response = await fetch(
          `${apiUrl}/google-calendar/status`,
          {
            method: 'GET',
            headers: {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      console.log(`Getting OAuth URL from: ${apiUrl}/google-calendar/get-auth-url`);
      
      try {
        // Get the OAuth URL from our edge function
        const response = await fetch(
          `${apiUrl}/google-calendar/get-auth-url`,
          {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${session.access_token}`,
        ...
        // Create a reference to the check function within this closure
        const checkConnectionFunc = checkConnection;
        ...
        const messageListener = async (event: MessageEvent) => {
          ...
          if (event.data === 'google_calendar_connected') {
            ...
            const isConnected = await checkConnectionFunc();
            if (isConnected) {
  ...
  const fetchEvents = useCallback(async (timeMin?: string, timeMax?: string) => {
    ...
    try {
      ...
      const url = new URL(`${apiUrl}/google-calendar/events`);
  ...
  const deleteEvent = useCallback(async (eventId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/google-calendar/delete?eventId=${eventId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${session.access_token}`,
