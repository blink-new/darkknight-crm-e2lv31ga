...
serve(async (req) => {
  ...
  try {
    ...
    console.log('GOOGLE_REDIRECT_URI exists:', !!googleRedirectUri);

    if (!supabaseUrl || !supabaseServiceKey || !googleClientId || !googleClientSecret || !googleRedirectUri) {
    ...
    switch (action) {
        ...
        try {
          ...
          if (Date.now() > expiresAt && integration.refresh_token) {
            try {
              ...
              return new Response(
                JSON.stringify({ error: 'Failed to refresh token', status: 401 }),
                { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
              );
            }
          }
      ...
      case 'update': 
      case 'delete': {
        // The implementation for these methods remains the same
        // We're keeping them but focusing on the most critical fixes right now
        return new Response(
          JSON.stringify({ error: 'Not implemented yet', status: 501 }),
