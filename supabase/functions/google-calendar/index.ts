...
serve(async (req) => {
  ...
  console.log('Request received for URL:', requestUrl);
  ...
  try {
    ...
    // Extract action from query parameter first
    const queryAction = url.searchParams.get('action');
    
    const pathParts = url.pathname.split('/');
    // Only use it as an action if it's not "google-calendar" (which would be the base endpoint)
    let action = 'default';
    
    // Check if we have a specific action in the URL
    if (pathParts.length > 0) {
      // Get the last part of the path as the action
      const lastPart = pathParts[pathParts.length - 1];
      
      // Only use it as an action if it's not "google-calendar" (which would be the base endpoint)
      if (lastPart && lastPart !== 'google-calendar') {
        action = lastPart;
      }
    }
    
    // If a query parameter action is provided, use it instead
    if (queryAction) {
      action = queryAction;
    }
    ...
    switch (action) {
      ...
      // Default response if no specific action is matched
      case 'default': {
        return new Response(
          JSON.stringify({ 
            service: 'Google Calendar Integration API',
            status: 'active',
            endpoints: [
